...
public class ProductCRUDRespository {
	public void insert_values(String itemId, String product_name, int quantity, String description) {
		...
		try {
			String query = "insert into PRODUCT(ITEM_ID, QUANTITY, PRODUCT_NAME,DESCRIPTION) values(?,?,?,?)";
	...
	public List<Product> get_values() {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				Product p = new Product(rs.getString("ITEM_ID"), rs.getString("PRODUCT_NAME"), rs.getInt("QUANTITY"),
	...
	public void edit_values(Product p) {
		...
		PreparedStatement ps = null;
		try {
			String querry = "update PRODUCT set ITEM_ID=?,QUANTITY=?,PRODUCT_NAME=?, DESCRIPTION=? where ITEM_ID=?";
			ps = connection.prepareStatement(querry);
